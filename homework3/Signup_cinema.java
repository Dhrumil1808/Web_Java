 ...
 */
@WebServlet("/cs1220/homework3/Signup_cinema")
public class Signup_cinema extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Signup_cinema() {
        super();
		...
		ArrayList<Login> users=(ArrayList<Login>) this.getServletContext().getAttribute("page");
		   ...
		   for(int i=0;i<users.size();i++)
		   {
			...
			if(valid)
			{
				...
				response.sendRedirect("signin_cinema.jsp");	
			}
			else
			{
				response.sendRedirect("Signup_cinema.jsp?msg=" + URLEncoder.encode(message,"UTF-8"));
			}
