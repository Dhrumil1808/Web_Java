 ...
 */
@WebServlet("/cs1220/homework3/signin_cinema")
public class signin_cinema extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public signin_cinema() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(int i=0;i<users.size();i++)
		{
			...
			if(users.get(i).getUsername().equalsIgnoreCase(username) && users.get(i).getPassword().equals(password))
			{
				...
				if(users.get(i).getUsername().equals("admin"))
				{
					...
					response.sendRedirect("admin_cinema");
					break;
				}
				else
				{
				...
				HttpSession sess=request.getSession();
				sess.setAttribute("trial",users.get(i).getFirstname());
				response.sendRedirect("welcome_cinema.jsp");
				break;
			}
			}
		...
		if(k==0){
			response.sendRedirect("signin_cinema.jsp?message="+ URLEncoder.encode("invalid login credentials","UTF-8"));
		}
